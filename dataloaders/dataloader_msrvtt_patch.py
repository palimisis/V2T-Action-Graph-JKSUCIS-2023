...
class MSRVTT_RawDataLoader(Dataset):
    ...
    Params:
        ...
        patch: Patch size. Default: None
        overlapped: Overlapping region size, in pixel. Default: 0
    """
    def __init__(
            self,
            csv_path,
...
#                           self.rawVideoExtractor.size, self.rawVideoExtractor.size), dtype=np.float)
        ...
        for i, video_id in enumerate(choice_video_ids):
            ...
            raw_video_data_patches,shapes = self.rawVideoExtractor.get_video_data(video_path, patch=self.patch, overlapped=self.overlapped)
            
            
            raw_video_data_patches = raw_video_data_patches['video']
            num_patches = self.patch*self.patch
            videos = []
            for p in range(num_patches):
                raw_video_data = raw_video_data_patches[:,p,:,:,:]
                shapes = raw_video_data.shape
                # Pair x L x T x 3 x H x W
                video = np.zeros((len(choice_video_ids), self.max_frames, 1, 3,
        ...
        return np.array(videos), video_mask
